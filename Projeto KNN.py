k = int(input('Digite o numero de vizinhos que queremos considerar: ')) #numero de vizinhos que queremos considerar

def distancia_euclidiana(p1,p2):
    soma = 0 # Inicializa a variável para armazenar o resultado 
    for i in range(len(p1)):  # Itera sobre cada característica dos pontos
        soma += (p1[i] - p2[i]) ** 2  # Calcula a diferença entre as coordenadas dos dois pontos, eleva ao quadrado e adiciona à soma
    return soma ** 0.5  # Tira a raiz quadrada da soma para obter a distância Euclidiana


def vizinhos(data, no_class):
    distancias = []  # Lista para armazenar as distâncias 
    
    #Calcular a distância entre o no_class e os que estão na base de dados
    for i in range(len(data)):
        distancia = distancia_euclidiana(data[i][2], no_class[2]) # Calcula a distancia entre as 2 listas
        distancias.append((data[i][1], distancia))  # Armazena o perfil e a distância na lista

    # Ordenar as distâncias em uma tupla de forma crescente
    def distancia_em_ordem(tupla):
        return tupla[1]  

    distancias.sort(key=distancia_em_ordem)  # Ordena a lista distancias usando a função distancia_em_ordem como chave
...
def classificar(vizinhos):
    contagem = {}  # Dicionário para contar a frequência de cada classe
    
    for vizinho in vizinhos:
        classe = vizinho[0]  # Obtém a classe do vizinho
        if classe in contagem:
            contagem[classe] += 1  # Incrementa a contagem da classe
        else:
            contagem[classe] = 1  # Inicializa a contagem da classe
...
# Para cada ponto de teste, classificar
for ponto in no_class:
    vizinhos_ponto = vizinhos(data, ponto)
    classe = classificar(vizinhos_ponto)
    print(f'O ponto {ponto[0]} é classificado como {classe}')
